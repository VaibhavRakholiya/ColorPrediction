...
async function CalculateWinnings(winningValue) {
  ...
  const filteredData = data?.data?.filter(entry => entry.game_type === 4) || []
  ...
  gameHistory.CreateGameHistory("RoundFour", spinStopNumber); // Creating game history to show on admin panel.
